...
public class WWWHelper : MonoBehaviour
{
	...
	//inhoi
	public delegate void HttpRq(int id,WWW www);

	public event HttpRq OnHttpR;
	...
	//	StartCoroutine(WaitForRequest(id, www));
	//}
	public void post2(int id, string url, Dictionary<string, string> data)
	{
		WWWForm form = new WWWForm();
	...
	public void subpost(int id, string url, string uid, string upw)
	{
		WWWForm form = new WWWForm();


			form.AddField("uid", uid);
			form.AddField("upw",upw);
		WWW www = new WWW(url, form);
		StartCoroutine(WaitForRequest(id, www));
	}
	public void post(int index,string url,string id,string Score){
		if (id != null && Score != null){
			if (id != " ")
			{
				WWWForm form = new WWWForm();
				Debug.Log("userName= " + id);
				Debug.Log("userScore" + Score);
				form.AddField("id", id);
				form.AddField("Score", Score);
				WWW www = new WWW(url, form);
				StartCoroutine(WaitForRequest(index, www));
			}
		}
	}
